...
@click.option('--reg_every',    help='lazy reg', metavar='int', type=click.FloatRange(min=1), default=4, required=False, show_default=True)
...
@click.option('--lighting_reg', help='Lighting regularization strength.', metavar='FLOAT', type=click.FloatRange(min=0), default=25.0, required=False, show_default=True)
@click.option('--smooth_reg', help='Smoothness regularization strength.', metavar='FLOAT', type=click.FloatRange(min=0), default=0.5, required=False, show_default=True)
...
@click.option('--hist_weight',    help='Histogram Color Loss weight.', metavar='FLOAT', type=click.FloatRange(min=0), default=10., required=False, show_default=True)
@click.option('--symmetry_weight',    help='Symmetry Loss weight.', metavar='FLOAT', type=click.FloatRange(min=0), default=1., required=False, show_default=True)
@click.option('--disambiguation_weight',    help='Disambiguation Loss weight.', metavar='FLOAT', type=click.FloatRange(min=0), default=10.0, required=False, show_default=True)
def main(**kwargs):
    """Train a GAN using the techniques described in the paper
    "Alias-Free Generative Adversarial Networks".
    ...
    c.G_kwargs = dnnlib.EasyDict(class_name=None, z_dim=512, w_dim=512, mapping_kwargs=dnnlib.EasyDict(), mapping_lit_kwargs=dnnlib.EasyDict(), synthesis_lit_kwargs=dnnlib.EasyDict())
    ...
    c.G_kwargs.synthesis_lit_kwargs.num_blocks = 2
    c.G_kwargs.synthesis_lit_kwargs.num_fp16_blk = 0
    c.G_kwargs.mapping_lit_kwargs.num_layers = opts.map_depth // 2
    ...
    c.G_kwargs.class_name = 'training.networks_stylegan_lit.TriPlaneGenerator'
    c.D_kwargs.class_name = 'training.dual_discriminator_lit.DualDiscriminator'
    ...
    if c.training_set_kwargs.resolution == 512:
        ...
        sr_module = 'training.superresolution.SuperresolutionHybrid4XDC'
    ...
    rendering_options = {
        ...
        'sr_antialias': True,
        'lighting_reg': opts.lighting_reg, # strength of lighting regularization
    }
    ...
    if opts.density_reg > 0 or opts.lighting_reg > 0 or opts.smooth_reg > 0 or opts.disambiguation_weight > 0:
        c.G_reg_interval = opts.reg_every
    ...
    c.loss_kwargs.lighting_weight = opts.lighting_reg
    c.loss_kwargs.smooth_weight = opts.smooth_reg
    c.loss_kwargs.hist_weight = opts.hist_weight
    c.loss_kwargs.symmetry_weight = opts.symmetry_weight
    c.loss_kwargs.disambiguation_weight = opts.disambiguation_weight
