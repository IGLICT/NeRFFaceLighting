...
class StyleGAN2Loss(Loss):
    ...
    def run_G(self, z, c, l, swapping_prob, neural_rendering_resolution, update_emas=False, **kwargs):
        ...
        gen_output = self.G.synthesis(ws, ws_lit, c, l, neural_rendering_resolution=neural_rendering_resolution, update_emas=update_emas, **kwargs)
    ...
    def accumulate_gradients(self, phase, real_img, real_c, real_l, gen_z, gen_c, gen_l, gain, cur_nimg):
        ...
        if phase in ['Gmain', 'Gboth']:
            with torch.autograd.profiler.record_function('Gmain_forward'):
                gen_img, _gen_ws, _gen_ws_lit = self.run_G(gen_z, gen_c, gen_l, swapping_prob=swapping_prob, neural_rendering_resolution=neural_rendering_resolution)
                gen_logits = self.run_D(gen_img, gen_c, gen_l, blur_sigma=blur_sigma)
                training_stats.report('Loss/scores/fake', gen_logits)
                training_stats.report('Loss/signs/fake', gen_logits.sign())
                loss_Gmain = torch.nn.functional.softplus(-gen_logits)
                training_stats.report('Loss/G/loss', loss_Gmain)
                training_stats.report('Stat/shade/mean', gen_img["image_shading"].mean(dim=(-1, -2)).squeeze())
        ...
        if phase in ['Greg', 'Gboth']:
            ...
            gen_img_s0 = self.G.synthesis(ws, ws_lit, gen_c, gen_l, neural_rendering_resolution=neural_rendering_resolution)
            
            # Mixing Another Latent Codes
            _ws = self.G.backbone.mapping(torch.randn_like(gen_z), c_gen_conditioning, truncation_psi=1., truncation_cutoff=None, update_emas=False)
            _ws[:, :7] = ws[:, :7] # Expect the geometry is similar
            
            _gen_img_s0 = self.G.synthesis(_ws, ws_lit, gen_c, gen_l, neural_rendering_resolution=neural_rendering_resolution, return_planes=True)
            
            gen_img_s0_hat = self.G.synthesis(ws, ws_lit, gen_c, gen_l, neural_rendering_resolution=neural_rendering_resolution, _planes_lit=_gen_img_s0['planes_lit']) # Replace the Shading Tri-plane
            
            # Same Shading, Cross Texture
            loss_Gcross_T = torch.nn.L1Loss()(gen_img_s0["image"], gen_img_s0_hat["image"])
            ...
            loss_Gcross_S = self.lpips_loss(gen_img_s1["image_albedo_raw"], ((gen_img_s1["image_raw"]/2+.5)/(gen_img_s0["image_shading"]+1e-5))*2-1)
        ...
        if phase in ['Dmain', 'Dboth']:
            with torch.autograd.profiler.record_function('Dgen_forward'):
                gen_img, _gen_ws, _gen_ws_lit = self.run_G(gen_z, gen_c, gen_l, swapping_prob=swapping_prob, neural_rendering_resolution=neural_rendering_resolution, update_emas=True)
                gen_logits = self.run_D(gen_img, gen_c, gen_l, blur_sigma=blur_sigma, update_emas=True)
                training_stats.report('Loss/scores/fake', gen_logits)
                training_stats.report('Loss/signs/fake', gen_logits.sign())
        ...
        if phase in ['Dmain', 'Dreg', 'Dboth']:
            ...
            with torch.autograd.profiler.record_function(name + '_forward'):
                ...
                real_logits = self.run_D(real_img_tmp, real_c, real_l, blur_sigma=blur_sigma)
