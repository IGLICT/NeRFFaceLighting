...
class HourglassBlock(nn.Module):
    ...
    def forward(self, x, light, count, skip_count):
        ...
        out_lower = F.interpolate(out_lower, scale_factor=2, mode='nearest')
...
class lightingNet(nn.Module):
    ...
    def forward(self, innerFeat, target_light, count, skip_count):
        ...
        innerFeat = torch.cat((upFeat, innerFeat[:, self.ncInput:]), dim=1)
...
class HourglassNet(nn.Module):
    ...
    def forward(self, x, target_light, skip_count):
        ...
        feat = nn.ReLU()(self.pre_bn(feat))
        # get the inner most features
        feat, out_light = self.HG3(feat, target_light, 0, skip_count)
        feat = nn.ReLU()(self.bn_1(self.conv_1(feat)))
        feat = nn.ReLU()(self.bn_2(self.conv_2(feat)))
        feat = nn.ReLU()(self.bn_3(self.conv_3(feat)))
